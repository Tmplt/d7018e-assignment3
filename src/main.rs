...
fn decode(wordarr: &[u32], bytearr: &mut [char], mut seed: &mut u32) -> u32 {
    let m: u32;
    let mut r: u32;
    let x: u32;
    let y: u32;
    ...
    if wordarr[0] == 0 {
        ...
        bytearr[0] = ((m & 0xff000) >> 13) as u8 as char;
        r = !codgen(&mut seed);
        r = r.wrapping_add(1);
...
fn main() {
    ...
    // let mut plain: [char; 132] = unsafe { mem::uninitialized() };
    let plain = vec![0 as char; 132];
    let mut plain = plain.into_boxed_slice();

    let mut seed: u32 = 0x0e0657c1;

    decode(&coded[..], &mut plain, &mut seed);

    let s: String = plain.into_iter().collect();
    println!("{}", s);
}
